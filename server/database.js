...
const getMetadata = async function (id) {
  ...
  if (id) return db.get(`SELECT Scheduler.cancelled, Metadata.id, datetime(Scheduler.start_time, 'unixepoch', 'localtime') as start_time, datetime(Scheduler.finish_time, 'unixepoch', 'localtime') as finish_time, datetime(Metadata.date, 'unixepoch', 'localtime') as date, Metadata.name, datetime(Scheduler.scheduled_time, 'unixepoch', 'localtime') as schedule, Metadata.data FROM Metadata LEFT JOIN Scheduler ON Metadata.id = Scheduler.metadata_id WHERE Metadata.id = ?`, [id])
  else return db.all(`SELECT Scheduler.cancelled, Metadata.id, datetime(Scheduler.start_time, 'unixepoch', 'localtime') as start_time, datetime(Scheduler.finish_time, 'unixepoch', 'localtime') as finish_time, datetime(Metadata.date, 'unixepoch', 'localtime') as date, Metadata.name, datetime(Scheduler.scheduled_time, 'unixepoch', 'localtime')  as schedule FROM Metadata LEFT JOIN Scheduler ON Metadata.id = Scheduler.metadata_id`)
}
...
const insertScheduler = async function (metadataID, scheduledTime) {
  const db = await dbPromise
  db.run('INSERT INTO Scheduler (metadata_id, scheduled_time) VALUES (?, ?);', [ metadataID, date(scheduledTime) ])
}
...
const insertMetadata = async function (data) {
  const db = await dbPromise
  db.get('INSERT OR REPLACE INTO Metadata (id, name, dos, data, date) VALUES (?, ?, ?, ?, ?);', [ data.id, data.name, data.dos, JSON.stringify({ urls: data.urls, dxcodes: data.dxcodes, postcodes: data.postcodes, errors: data.errors, auth: data.auth }), date(data.date) ])
  insertScheduler(data.id, data.schedule)
}
